...
public class Aeroporto extends javax.swing.JFrame {
    ...
    public Aeroporto() throws InterruptedException {
        initComponents();
        //modelos das tabelas
    ...
    private void initComponents() {
        ...
        jButtonIniciar.setText("Adicionar Avião");
    ...
    private void iniciar() {
        ...
        while (!filaPouso.isEmpty() || !filaEstacionamento.isEmpty() || !filaTaxiamento.isEmpty() || !filaGate.isEmpty()) {
            ...
            //se a filaPouso for <5 vai dar erro, pq ele vai preencher 5 aviões no gate por causa do for, e não tem 5 aviões na filaPouso
            if (filaPouso.size() >= 5) {
                if (teste == false) {
                    if (!filaPouso.isEmpty()) {
                        //entra 1 vez só nesse for, para preencher todos os gates
                        for (int i = 0; i < 5; i++) {
                            pousoParaGate();
                            starvation("gasolina");
            ...
            //a cada 3 pousos 1 decola
            if (qtdPouso % 3 == 0 || filaPouso.isEmpty()) {
    ...
    private void starvation(String tipoStarvation) {
        if (tipoStarvation.equals("gasolina")) {
            for (Aviao aviao : filaPouso) {
                ...
                if (aviao.getQuantidadeCombustivel() < 40) {
